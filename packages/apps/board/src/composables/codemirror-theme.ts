...
// Ported from https://github.com/unocss/unocss/blob/main/packages-integrations/inspector/client/theme.ts
// (the theme used by the UnoCSS Playground editor), with tags added for JSON mode:
// `t.null` so that `null` matches the `true`/`false` literal colors, and
// `t.squareBracket` so that `[`/`]` match the `t.bracket` color used in JS mode.
...
export const vitesseHighlightStyle = HighlightStyle.define([
  ...
  {
    tag: [t.bracket, t.squareBracket],
    color: "#4d9375",
  },
  {
    tag: [t.typeName, t.className, t.number, t.changed, t.annotation, t.modifier, t.self, t.namespace, t.keyword, t.atom, t.bool, t.null, t.special(t.variableName)],
    color: keyword,
  },
  {
