...
class Balloon extends React.Component {
  ...
  getTableData(contest_config: any, team: any, run: Run[]) {
    if (contest_config?.balloon_color == null) return [];
    ...
    const uniqueTeamProblemSolved = new Set();

    let first_solved = new Set();

    run
      .filter((x) => isAccepted(x.status))
      .forEach((run: Run) => {
        if (
          uniqueTeamProblemSolved.has(getSolvedId(run.teamId, run.problemId))
        ) {
          return;
        }

        uniqueTeamProblemSolved.add(getSolvedId(run.teamId, run.problemId));
        ...
        const status = balloon_dispatched_set.has(
          getSolvedId(run.teamId, run.problemId),
        );

        let tableItem: BalloonTableItem = {
          status: status,
          time: run.timestamp,
          solved: run.problemId,
          total: deepCopy(team[run.teamId].total),
          action: {
            status: status,
            solvedId: getSolvedId(run.teamId, run.problemId),
          },
          team: team[run.teamId].name,
          awards: awards,
        };

        if (contest_config?.badge) {
          tableItem.badge = team[run.teamId].badge?.base64;
        }

        if (contest_config?.organization) {
          tableItem.organization = team[run.teamId].organization;
        }
